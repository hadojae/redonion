#!/bin/bash
#######################################
# Red Onion Install Script v0.1
# https://github.com/hadojae/redonion
# tested on Centos/RHEL 6.6
########################################
...
sniff_int="eth1"            			# Whatever interface you will use for sniffing  
manage_int="eth0"           			# Whatever interface you will use for management  
manage_ip="10.10.2.113"      			# Management IP address                     
user_drop="ro"          			# A user account to drop permissions to during install
wrk_dir="/home/ro/redonion-master"        	# The directory you will be installing from 
...
echo -e ""
echo -e "\x1B[01;32m           REDONION v0.1 {https://github.com/hadojae/redonion}            \x1B[0m"
space_pls
}
...
function letsgo () 
{
  ...
  # Check for known bad kernel
  if [ `uname -r` == "2.6.32-504.el6.x86_64" ]; then
      print_error "It looks like you need to update and reboot. This kernel is no good for pf_ring stuff."
      exit 0
  fi
  
  # Make sure that we are running on RHEL or Centos
  if [[ ! -f /etc/redhat-release && ! -f /etc/centos-release ]]; then
    print_error "I'm sorry, this distibution is not yet supported."
    exit 0
  fi   
